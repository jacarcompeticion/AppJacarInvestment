...
st.set_page_config(page_title="Jacar Pro V39", layout="wide", page_icon="🏦")
...
def auto_analizar(t, n):
    try:
        ...
        def extraer(tag):
            # Buscamos la línea que empieza por el TAG
            for line in res_ia.split('\n'):
                if tag in line.upper():
                    # Limpiamos y dividimos por el separador |
...
# --- 3. INTERFAZ: PESTAÑAS HORIZONTALES ---
...
with t_main[0]: # STOCKS
    s1, s2, s3, s4, s5 = st.tabs(["💻 Tecnología", "⛽ Energía", "🏦 Banca", "🛒 Consumo", "🇪🇸 España"])
    with s1: grid({"🍏 Apple":"AAPL", "🤖 NVDA":"NVDA", "🚗 Tesla":"TSLA", "🔍 Google":"GOOGL", "📦 Amazon":"AMZN"})
    with s2: grid({"⛽ Exxon":"XOM", "🐚 Shell":"SHEL", "🔥 Chevron":"CVX"})
    with s3: grid({"💳 Visa":"V", "🏦 JPMorgan":"JPM", "📈 Goldman":"GS"})
    with s4: grid({"🥤 Coca-Cola":"KO", "🍔 McDonald's":"MCD", "🛒 Walmart":"WMT"})
    with s5: grid({"👕 Inditex":"ITX.MC", "⚡ Iberdrola":"IBE.MC", "🏦 Santander":"SAN.MC", "🏦 BBVA":"BBVA.MC"})
...
if not df.empty:
    ...
    # EMA COLOR NARANJA PARA DISTINCIÓN
...
if st.session_state.analisis_auto:
    ...
    for i, tag in enumerate(["intra", "medio", "largo"]):
        s = res[tag]
        with cols_ia[i]:
            st.markdown(f"""<div style="background-color:white; padding:15px; border-radius:10px; border:1px solid #ddd; height:180px;">
                <h4 style="text-align:center; color:#268bd2; margin:0;">{tag.upper()} ({s[0]})</h4>
                <p style="text-align:center; font-weight:bold; margin:5px;">{s[1]}</p>
                <p style="margin:2px;">📦 Lotes: {s[2]} | In: <b>{s[3]}</b></p>
                <p style="margin:2px;">🏁 TP: <span style="color:green;">{s[4]}</span> | 🛡️ SL: <span style="color:red;">{s[5]}</span></p>
        ...
        for i, pos in enumerate(list(st.session_state.cartera_abierta)):
            with st.expander(f"📌 {pos['activo']} ({pos['tipo']})"):
                ...
                if st.button("Cerrar", key=f"c_{pos['id']}", use_container_width=True):
                    # Guardar en histórico antes de borrar
                    st.session_state.historial.append({
                        "fecha": datetime.now().strftime("%d/%m %H:%M"),
                        "activo": pos['activo'], "pnl": pnl
                    })
                    st.session_state.wallet += pnl
                    st.session_state.cartera_abierta.pop(i)
                    guardar_datos(st.session_state.cartera_abierta, CSV_FILE)
                    guardar_datos(st.session_state.historial, HIST_FILE)
                    st.rerun()

    with tab_side[1]:
        if st.session_state.historial:
            df_hist = pd.DataFrame(st.session_state.historial).iloc[::-1] # Ver últimos primero
            ...
            st.write("No hay cierres registrados.")
